...
| ID / 判定 | 条件・手順 | 期待結果 |
| --- | --- | --- |
| GA-01 [OK] | 未接続で ChatGPT を開き、GitHub パネルを確認する | 未接続メッセージと「設定を開く」を表示し、PAT 入力欄はパネルに存在しない |
| GA-02 [NG] | 「設定を開く」を選ぶ | 拡張の設定画面が開く |
...
| ID / 判定 | 条件・手順 | 期待結果 |
| --- | --- | --- |
| GU-01 [ ] | ChatGPT を開く | Dropbox は右、GitHub は左に独立して表示され、各パネルを個別に開閉できる |
| GU-02 [ ] | 未接続状態で GitHub パネルを開く | 未接続の理由と設定画面への導線を表示し、読み込みボタンを無効にする |
...
| GU-12 [ ] | 狭い画面幅で Dropbox と GitHub を表示する | ChatGPT 入力欄やパネル操作を妨げず、パネルが画面外へ出ない。重なりが起きる場合は不具合として記録する |
...
| GP-08 [ ] | Dropbox と GitHub でそれぞれ選択して読み込む | 各パネルはサービス専用プロンプトを作り、同一プロンプトに混在させない |
...
v0.2.0 では Dropbox の認証、API、UI、プロンプト形式を変更しない。GitHub UI の mount や共通 auto-send が Dropbox を壊していないことを確認する。
...
| DR-07 [ ] | GitHub と Dropbox の両パネルを開閉する | runtime message、storage key、画面位置、選択状態が相互に干渉しない |
