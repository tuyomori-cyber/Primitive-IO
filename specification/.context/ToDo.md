...
- [ ] 内部状態はDropboxのファイルIDを主に使い、表示・プロンプトには表示用パスを使う
...
- [ ] 単一ファイル、複数ファイル（5件）、深い階層、空フォルダを手動テストする
- [ ] 未認証、認証拒否、トークン期限切れ、Dropbox API失敗、ChatGPT入力欄未検出をテストする
- [ ] 日本語ファイル名・パス、同名ファイル、非対応拡張子をテストする
- [ ] ChatGPT側Dropbox連携が同一または同等権限のアカウントであることを確認する手順をREADMEへ記載する
- [ ] プライバシー説明、必要権限、接続解除方法、既知のDOM依存をREADMEへ記載する
- [ ] Firefoxでのパッケージ化・インストール手順を作成する
- [ ] 署名済みまたはFirefoxで恒久インストール可能なXPIで、Firefox再起動後にDropbox接続状態を復元できることを確認する
